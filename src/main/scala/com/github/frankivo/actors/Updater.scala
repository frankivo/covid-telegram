...
object Updater {
  ...
  def dateRange(start: LocalDate = COVID_EPOCH): Seq[LocalDate] = {
    val dayCounts = Duration
      .between(start.atStartOfDay(), LocalDate.now().atStartOfDay())
      .toDays
  ...
  val URL_NATIONAL: String =
    "https://raw.githubusercontent.com/mzelst/covid-19/master/corrections/corrections_per_day/corrections-%s.csv"
}

/** Downloads CSV data from GitHub. This data contains daily national covid
  * statistics.
  */
class Updater extends Actor {
  ...
  private def onMessage(hasRun: Boolean): Receive = {
    ...
    case r: RequestSource =>
      CovidBot.ACTOR_TELEGRAM ! TelegramText(
        r.destination,
        s"Source: ${Updater.URL_SOURCE}"
      )
  }

  /** Downloads all the CSV files that are not yet present on the local
    * filesystem.
    *
    * @param hasRun
  ...
  /** Downloads a file into a directory.
    *
    * @param date
    *   The date to download statistics for.
    */
