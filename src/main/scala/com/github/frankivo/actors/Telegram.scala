...
object Telegram {
  ...
  def registerUpdates: Boolean =
    sys.env.getOrElse("TELEGRAM_UPDATES", "true").toBoolean

  def isOwner(destination: Long): Boolean = ownerId == destination
}

/** Connects to the Telegram API. Sends out broadcast messages and handles
  * requests from Telegram members.
  */
class Telegram extends Actor {
  ...
  private def handleUpdates(updates: Seq[Update]): Unit = {
    val commands = updates
      ...
      .filter(u =>
        u.entities.exists(e => e.`type`.eq(MessageEntity.Type.bot_command))
      )
      .map(u => {
  ...
  private def handleCommands(commands: Seq[Command]): Unit = {
    commands
      .foreach(c => {
        c.cmd match {
          case "/ath" =>
            CovidBot.ACTOR_STATS ! RequestAllTimeHigh(c.destination)
          case "/cases" =>
            CovidBot.ACTOR_STATS ! RequestCasesForDate(
              c.destination,
              c.parameter
            )
          case "/graph" =>
            CovidBot.ACTOR_GRAPHS ! RequestRollingGraph(c.destination)
          case "/hi" => send(TelegramText(c.destination, "Hi!"))
          case "/month" =>
            CovidBot.ACTOR_GRAPHS ! RequestMonthGraph(
              c.destination,
              c.parameter
            )
          case "/refresh" =>
          ...
          case "/weekly" =>
            CovidBot.ACTOR_GRAPHS ! RequestWeekGraph(c.destination, c.parameter)
  ...
  def send(txt: TelegramText): Unit =
    bot.execute(new SendMessage(txt.destination, txt.body))

  def send(img: TelegramImage): Unit =
    bot.execute(new SendPhoto(img.destination, img.file))
